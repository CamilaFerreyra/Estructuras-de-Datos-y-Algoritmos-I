...
static AVL_Nodo* avl_nodo_rotacion_simple_izq(AVL_Nodo* raiz) {
  ...
  hijoDer->izq = raiz;
  // actualizar alturas
  raiz->altura = 1 + avl_nodo_max_altura_hijos(raiz); //cambia su altura
  hijoDer->altura = 1 + avl_nodo_max_altura_hijos(hijoDer); //depende de la altura de la raiz
  return hijoDer;
}
...
static AVL_Nodo* avl_nodo_insertar(AVL_Nodo* raiz, void* dato,
  ...
  if (raiz == NULL) // insertamos el nuevo elemento
    ...
    // chequear balance
    if (avl_nodo_factor_balance(raiz) == -2) { //Insertar a izq desbalancea a izq (<0)
      // casos 1 o 2
      if (avl_nodo_factor_balance(raiz->izq) == 1) // caso 2
        raiz->izq = avl_nodo_rotacion_simple_izq(raiz->izq); //rotacion sobre el hijo izq que está balanceado
      raiz = avl_nodo_rotacion_simple_der(raiz); // caso 1 //factor -1, desbalanceado por fuera, todo izi
    }
    raiz->altura = 1 + avl_nodo_max_altura_hijos(raiz);
    return raiz;
  }
    ...
    //check balance
    if (avl_nodo_factor_balance(raiz) == 2) { //Porque es derecho menos izq
      if(avl_nodo_factor_balance(raiz->der) == -1) //caso 2: tengo que subir la raiz balanceada para que quede el desbalance por fuera
        raiz->der = avl_nodo_rotacion_simple_der(raiz->der);
      raiz = avl_nodo_rotacion_simple_izq(raiz); //caso 1 : ya tengo el subarbol pesado desbalanceado por fuera, exterior
    }
    raiz->altura = 1 + avl_nodo_max_altura_hijos(raiz);
    return raiz;
...
AVL_Nodo *get_min_mayores(AVL_Nodo* raiz) {
  AVL_Nodo *der = raiz->der;
  if(der == NULL) return raiz;
  if(der->izq != NULL) return get_min_mayores(der->izq);
}

AVL_Nodo *avl_nodo_eliminar (AVL_Nodo *raiz, void *dato, FuncionComparadora comp, FuncionDestructora destr) {
  if (raiz == NULL) return raiz;
    ...
    //magia eliminadora del nodo
    if (raiz->izq == NULL) {
      AVL_Nodo *nuevaRaiz = raiz->der;
      destr(raiz->dato);
      free(raiz);
      return nuevaRaiz;
    } else if(raiz->der == NULL) {
      AVL_Nodo *nuevaRaiz = raiz->izq;
      destr(raiz->dato);
      free(raiz);
      return nuevaRaiz;
    } else {
      //tomar el menor de los mayores 
      AVL_Nodo* nuevaRaiz = get_min_mayores(raiz);
...
void avl_eliminar (AVL arbol, void *dato) {
  arbol = avl_nodo_eliminar(arbol, dato, arbol->comp, arbol->destr);
}
